...
//   setInterval(() => { // 数秒置き
//     let target = currentIndex + 1; // nextと同じ動きにしている
...
window.addEventListener('load', () => {
  ...
  setInterval(() => {
    ...
    let target = Math.floor(Math.random() * images.length); // 毎回同じ順番ではなくランダムにactiveされる
    // if (target === images.length) {
    //   target = 0;
    // }
    document.querySelectorAll('.carousel__thumbnails > li')[target].click();
...
// 即時関数 スコープを絞ることができる
(function immediate() { // 即時関数にすることで設定と同時に実行される
  console.log('即時関数です');
})();

// for(let i = 0; i < 10; i++) {
  // console.log(Math.floor(Math.random() * 7)); // Math.foorを加えると小数点以下は切り捨てられる
// }
